{ config, lib, pkgs-default, ... }:
{
	...
	config = lib.mkIf (config.packages.discord.enable && config.packages.discord.variation == "nixcord") {
		
		home-manager.users.${config.user.settings.username} = { config, lib, pkgs-default, ... }: {
			programs.nixcord = {
				enable = true;
				vesktop = {
					enable = true;
