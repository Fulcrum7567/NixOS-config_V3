  ...
  config = lib.mkIf (cfg.enable && (cfg.activeConfig == "roundcube")) {
    ...
    services.roundcube = {
      ...
      extraConfig = ''
        ...
        $config['smtp_server'] = 'ssl://${mailServerCfg.fullDomainName}';
        ...
        $config['oauth_client_secret'] = file_get_contents('${config.sops.secrets."roundcube/oauth/client_secret".path}');
