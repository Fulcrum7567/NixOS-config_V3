{ config, lib, ... }:
{
  options.server.services.mail-client = {
    enable = lib.mkOption {
      type = lib.types.bool;
      default = false;
    ...
    activeConfig = lib.mkOption {
      type = lib.types.enum (config.server.services.mail-client.availableConfigs or []);
      default = "roundcube";
    ...
    fullDomainName = lib.mkOption {
      type = lib.types.str;
      default = "${config.server.services.mail-client.subdomain}.${config.server.webaddress}";
      description = "The full domain name to access the Email service.";
    };

    fullHttpsUrl = lib.mkOption {
      type = lib.types.str;
      default = "https://${config.server.services.mail-client.fullDomainName}";
